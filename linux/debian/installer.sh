...
if [ $dependencies_available = true ]; then
    ...
    if [ "$user_selection" = "Y" ]; then
        ...
        if [ $root_check -eq 0 ]; then
            install_script="sudo apt-get update -y; sudo apt-get install "
            install_zlib=false
            ...
            if [ $install_zlib = true ]; then
                install_script="$install_script; mkdir ansys-prereq; cd ansys-prereq; wget https://zlib.net/current/zlib.tar.gz; tar xvzf zlib.tar.gz; cd zlib-*; make clean; ./configure; make; sudo make install; cd ../..; rm -rf ansys-prereq;"
            fi
            eval $install_script
            dpkg -x ./ansys_python_manager.deb ${HOME}/.local
            ./postInstallScript.sh
