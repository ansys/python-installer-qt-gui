...
CONSTANTS_FILE = os.path.join(ROOT_DIR, "src", "ansys", "tools", "installer", "constants.py")
...
for python_version_key, python_version_value in vanilla_python_versions.items():
    ...
    patch_versions = get_minor_version_sublist_with_greater_patch(ftp_versions, python_version_value)

    # Check if the patch versions contain the executable
    new_patch_version = None
    for patch_version in patch_versions:
        # Check if the executable exists
        response_1 = requests.get(f"{PYTHON_FTP}/{patch_version}/Python-{patch_version}.tar.xz")
        response_2 = requests.get(f"{PYTHON_FTP}/{patch_version}/python-{patch_version}-amd64.exe")
...
count = 0
for asset in assets:
    if f"Miniforge3-{latest_conda_release["name"]}-Linux-x86_64.sh" == asset["name"]:
        count += 1
